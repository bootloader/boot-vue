...
import { paginateArray } from "@app-pushapp/@fake-db/utils";
...
mock.onGet("/apps/invoices").reply((config) => {
  ...
  if (sort.length) {
    if (sort[0]?.key === "client") {
      ...
      filteredInvoices = filteredInvoices.sort((a, b) => {
        if (sort[0]?.order === "asc") return a.total - b.total;

        return b.total - a.total;
      });
    } else if (sort[0]?.key === "id") {
      filteredInvoices = filteredInvoices.sort((a, b) => {
        if (sort[0]?.order === "asc") return a.id - b.id;
      ...
      filteredInvoices = filteredInvoices.sort((a, b) => {
        if (sort[0]?.order === "asc") return a.balance - b.balance;
  ...
  return [
    200,
    {
      invoices: paginateArray(filteredInvoices, itemsPerPage, page),
      totalInvoices,
      page: page > Math.ceil(totalInvoices / itemsPerPage) ? 1 : page,
    },
  ];
});
