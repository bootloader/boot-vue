...
import { genId, paginateArray } from "@app-pushapp/@fake-db/utils";
...
mock.onGet("/apps/users/list").reply((config) => {
  ...
  if (sort.length) {
    ...
    if (sort[0]?.key === "billing") {
      filteredUsers = filteredUsers.sort((a, b) => {
        if (sort[0]?.order === "asc") return a.billing.localeCompare(b.billing);
        else return b.billing.localeCompare(a.billing);
      });
    }
    if (sort[0]?.key === "role") {
      filteredUsers = filteredUsers.sort((a, b) => {
        if (sort[0]?.order === "asc") return a.role.localeCompare(b.role);
    ...
    if (sort[0]?.key === "status") {
      filteredUsers = filteredUsers.sort((a, b) => {
        if (sort[0]?.order === "asc") return a.status.localeCompare(b.status);
