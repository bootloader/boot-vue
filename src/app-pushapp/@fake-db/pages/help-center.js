...
// Images
import discord from "@app-pushapp/assets/images/svg/discord.svg";
import gift from "@app-pushapp/assets/images/svg/gift.svg";
import keyboard from "@app-pushapp/assets/images/svg/keyboard.svg";
import laptop from "@app-pushapp/assets/images/svg/laptop.svg";
import lightbulb from "@app-pushapp/assets/images/svg/lightbulb.svg";
import rocket from "@app-pushapp/assets/images/svg/rocket.svg";
...
const data = {
  popularArticles: [
    ...
    },
    {
      ...
      img: keyboard,
      subtitle:
        "This article will show you how to expand the functionality of App",
    },
  ],
  categories: [
    {
...
mock.onGet("/pages/help-center/article").reply((config) => {
  ...
  const activeCategory = data.categories.filter(
    (item) => item.slug === category
  )[0];
  const activeSubcategory =
    activeCategory.subCategories.filter(
      (item) => item.slug === subcategory
    )[0] || activeCategory.subCategories[0];
  const activeArticle = activeSubcategory.articles.filter(
    (item) => item.slug === article
  )[0];
