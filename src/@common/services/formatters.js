...
const isTodayDomain = (someDate) => {
  ...
  return (
    moment(new Date(someDate)).get("date") ==
      moment(new Date(today)).get("date") &&
    moment(new Date(someDate)).get("month") ==
      moment(new Date(today)).get("month") &&
    moment(new Date(someDate)).get("year") ==
      moment(new Date(today)).get("year")
  );
};
...
function phoneFormatted(value) {
  ...
  if (/^[1-9]{1}[0-9]{3,14}$/.test(value)) return true;
  ...
  if (/^(\+\d{1,3}( )?)?((\(\d{3}\))|\d{3})[- .]?\d{3}[- .]?\d{4}$/.test(value))
    return true;
  // 202 555 0125
  if (/^(\d{3}[- .]?){2}\d{4}$/.test(value)) return true;
  //(202) 555-0125
  if (/^((\(\d{3}\))|\d{3})[- .]?\d{3}[- .]?\d{4}$/.test(value)) return true;
  return false;
}
...
var formatter = {
  ...
  thumburl: function (mediaUrl, w, h) {
    ...
    var m = mediaUrl.match(
      /(.+)\/(res.cloudinary.com)\/([a-zA-Z0-9-_]+)\/([a-zA-Z0-9]+)\/(upload)\/([a-zA-Z0-9,_-]+)\/(.*)/
    );
    if (m && m.length) {
      m[6] = `w_${_w},h_${_h}`;
  ...
  phoneML: function phoneValidator(lineString) {
    ...
    for (var i in lines) {
      if (
        lines[i] &&
        !phoneFormatted(lines[i]) &&
        !phoneFormatted(lines[i].replace(/[\ \+]/g, ""))
      )
  ...
  HBVar: function ({ contents, position, min, max, vars, ordered }) {
    ...
    if (min) {
      min = parseInt(min);
      if (myArray.length < min) return "errors.LessVariable";
    }
    if (max) {
      max = parseInt(max);
  ...
  init: function () {
    ...
    Vue.filter("display_lane", function (contact) {
      return (
        Object.values(contact).filter((prop) => {
          return !!prop?.lane;
        })[0]?.lane || contact.lane
      );
    });
